...
class Glycol{
    ...
    protected function initTimer() {
        swoole_timer_tick($this->conf['emit_interval']*1000, function () {
    ...
    private function msgData($product_id, $user_id, $record) {
        ...
        if(empty($record)){
            ...
            foreach ($record as $key => $value) {
                ...
                $tmp[$key . '_average'] = array_merge(Calculate::arraySummary($value),[$this->timestamp]);
            }
        }
        //增加开盘信息
    ...
    private function storeRecords($new_records) {
        ...
        foreach ($new_records as $record) {
            ...
            //清理过期数据
            Calculate::removeExpireData($this->records[$product_id][$user_id][$trade_type], $product_id, $user_id, $trade_type);
        }
    ...
    protected function initClientWorker() {
        // 初始化与gateway连接服务
        $this->client_worker = new ClientWorker($this->conf['gateway_addr'], Calculate::groupIno());
        // 消息回调
    ...
    public function onGatewayMessage($connection, $data) {
        ...
        if (empty($json)) {
            ...
            if (isset($json->business_type) && $json->business_type == 'firstLogin' && !empty($json->client) && !empty($json->notify_fd) && !empty($json->product_id) && isset($json->user_id)) {
                //用户来获取第一次登录后的实时信息了
                if (isset($json->company_id) && !empty($json->company_id)){
                    $json = $this->firstLoginDataNotify($json->product_id, $json->user_id, $json->client, $json->notify_fd, $json->company_id);
                } else {
                    $json = $this->firstLoginDataNotify($json->product_id, $json->user_id, $json->client, $json->notify_fd);
                }
            }
        }else if(isset($json->id) && $json->id == MsgIds::MESSAGE_JOIN_SUCCESS) {
            //加入推送组
            //echo $json->message."\n";
        }
    }
    ...
    public function workerStart() {
        //Process::daemon(false,true);//后台运行
    ...
    public function __construct() {
        ...
        //加载配置
        $conf = include __DIR__ . '/../../config/gateway.php';
        $this->conf = $conf;
        //初始化品类
