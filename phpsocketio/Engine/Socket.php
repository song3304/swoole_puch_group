...
class Socket extends Emitter
{
    ...
    public function maybeUpgrade($transport)
    {
		...
		//SamLuo
		$this->upgradeTimeoutTimer =$this->server->server->tick($this->server->upgradeTimeout*1000, array($this, 'upgradeTimeoutCallback'),$transport);
    ...
    public function onUpgradePacket($packet)
    {
        ...
        if('ping' === $packet['type'] && (isset($packet['data']) && 'probe' === $packet['data']))
        {
            ...
            if ($this->checkIntervalTimer) {
				//SamLuo
				$this->server->server->clearTimer($this->checkIntervalTimer);
	        }
           // $this->checkIntervalTimer = Timer::add(0.5, array($this, 'check'));
		   // 
			//SamLuo
			$this->checkIntervalTimer =$this->server->server->tick(500, array($this, 'check'));
        }
    ...
    public function upgradeCleanup()
    {
		...
		//SamLuo
		$this->checkIntervalTimer && $this->server->server->clearTimer($this->checkIntervalTimer);
		$this->checkIntervalTimer = NULL;
		$this->upgradeTimeoutTimer && $this->server->server->clearTimer($this->upgradeTimeoutTimer);
    ...
    public function upgradeTimeoutCallback($timer_id,$transport)
    {
		//SamLuo
		$this->server->server->clearTimer($timer_id);
    ...
    public function setPingTimeout()
    {
		if ($this->pingTimeoutTimer) {
			$this->server->server->clearTimer($this->pingTimeoutTimer);
        }
		//SamLuo
		$this->pingTimeoutTimer = $this->server->server->after(($this->server->pingInterval + $this->server->pingTimeout)*1000 , array($this, 'pingTimeoutCallback'));
    }
    ...
    {
        //echo "reson:$reason,description:$description\n";
